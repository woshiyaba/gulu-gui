...
async def list_dicts(
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            ...
            await cur.execute(
                f"""
                SELECT id, dict_type, code, label, extra, sort_order
                FROM sys_dict
                {where_clause}
...
async def get_dict_by_id(dict_id: int) -> dict | None:
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT id, dict_type, code, label, extra, sort_order FROM sys_dict WHERE id = %s",
                (dict_id,),
            )
            return await cur.fetchone()
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id, dict_type, code, label, extra, sort_order
                """,
                (payload["dict_type"], payload["code"], payload["label"],
                 payload.get("extra", ""), payload["sort_order"]),
            )
            row = await cur.fetchone()
        await conn.commit()
        return row
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                """
                UPDATE sys_dict
                SET dict_type = %s, code = %s, label = %s, extra = %s, sort_order = %s
                WHERE id = %s
                RETURNING id, dict_type, code, label, extra, sort_order
                """,
                (
...
async def list_pokemon_marks_for_ops(
    ...
    """按 zh_name / key 关键字分页查询 pokemon_mark。"""
    pool = await get_pool()
    conditions: list[str] = []
    params: list = []
    kw = keyword.strip()
    if kw:
        conditions.append("(zh_name LIKE %s OR key LIKE %s)")
...
async def get_pokemon_mark_by_id(mark_id: int) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
...
async def get_pokemon_mark_by_key(key: str) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT id, key, zh_name, sort_order FROM pokemon_mark WHERE key = %s",
                (key,),
            )
            return await cur.fetchone()


async def get_pokemon_mark_by_sort_order(sort_order: int) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT id, key, zh_name, sort_order FROM pokemon_mark WHERE sort_order = %s",
                (sort_order,),
            )
            return await cur.fetchone()


async def create_pokemon_mark(payload: dict) -> dict:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                """
                INSERT INTO pokemon_mark (key, zh_name, zh_description, sort_order, image)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id, key, zh_name, zh_description, sort_order, image
                """,
                (
                    payload["key"],
                    payload["zh_name"],
                    payload.get("zh_description", ""),
...
async def update_pokemon_mark(mark_id: int, payload: dict) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                """
                UPDATE pokemon_mark
                SET key = %s, zh_name = %s, zh_description = %s, sort_order = %s, image = %s
                WHERE id = %s
                RETURNING id, key, zh_name, zh_description, sort_order, image
                """,
                (
...
async def delete_pokemon_mark(mark_id: int) -> bool:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
...
async def get_evolution_graph_by_chain_id_for_ops(chain_id: int) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        return await _get_evolution_graph_by_chain_id(conn, chain_id)


async def get_evolution_graph_by_pokemon_id_for_ops(pokemon_id: int) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT id, chain_id FROM pokemon WHERE id = %s", (pokemon_id,))
            row = await cur.fetchone()
            if not row:
                return None
            chain_id = row.get("chain_id")
            if chain_id is None:
                return {"chain_id": None, "nodes": [], "edges": []}
        return await _get_evolution_graph_by_chain_id(conn, int(chain_id))


def _detect_cycle(node_ids: set[int], edges: list[dict]) -> bool:
    indegree: dict[int, int] = {node_id: 0 for node_id in node_ids}
    graph: dict[int, list[int]] = defaultdict(list)
    for edge in edges:
        src = int(edge["pre_pokemon_id"])
        dst = int(edge["pokemon_id"])
...
async def save_evolution_graph_by_pokemon_id_for_ops(pokemon_id: int, payload: dict) -> dict | None:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT id, chain_id FROM pokemon WHERE id = %s", (pokemon_id,))
            ...
            if not anchor:
                return None
            current_chain_id = anchor.get("chain_id")

            node_ids = sorted({int(node.get("pokemon_id")) for node in (payload.get("nodes") or []) if node.get("pokemon_id")})
            edges: list[dict] = []
            seen_edges: set[tuple[int, int]] = set()
            for edge in payload.get("edges") or []:
                src = int(edge.get("pre_pokemon_id") or 0)
                dst = int(edge.get("pokemon_id") or 0)
                if src <= 0 or dst <= 0:
                    continue
                key = (src, dst)
                ...
                edges.append(
                    {
                        "pre_pokemon_id": src,
                        "pokemon_id": dst,
                        "pre_evolution_condition": (edge.get("pre_evolution_condition") or "").strip(),
                    }
                )

            if not node_ids:
                if current_chain_id is not None:
                    await cur.execute("DELETE FROM pokemon_evolution_chain WHERE chain_id = %s", (current_chain_id,))
                    await cur.execute("UPDATE pokemon SET chain_id = NULL WHERE chain_id = %s", (current_chain_id,))
                    await conn.commit()
                return {"chain_id": None, "nodes": [], "edges": []}

            if pokemon_id not in node_ids:
                node_ids.append(pokemon_id)
                node_ids.sort()

            node_set = set(node_ids)
            for edge in edges:
                if edge["pre_pokemon_id"] == edge["pokemon_id"]:
                    raise ValueError("自环关系不允许保存")
                if edge["pre_pokemon_id"] not in node_set or edge["pokemon_id"] not in node_set:
                    raise ValueError("进化关系必须连接到当前画布中的节点")

            if _detect_cycle(node_set, edges):
                raise ValueError("进化关系存在闭环，请移除循环关系后再保存")

            placeholders = ", ".join(["%s"] * len(node_ids))
            await cur.execute(
                f"SELECT id, name, chain_id FROM pokemon WHERE id IN ({placeholders})",
                node_ids,
            )
            pokemon_rows = await cur.fetchall()
            exists_ids = {int(row["id"]) for row in pokemon_rows}
            missing = sorted(node_set - exists_ids)
            if missing:
                raise ValueError(f"存在无效精灵ID: {', '.join(str(x) for x in missing)}")

            # 严格模式：一个 pokemon 只能属于一条链，禁止把其他链成员直接拖入当前链保存。
            # 允许的情况：
            # 1) 当前编辑链中的成员（chain_id == current_chain_id）
            # 2) 尚未入链成员（chain_id is NULL）
            conflicts: list[str] = []
            for row in pokemon_rows:
                row_chain_id = row.get("chain_id")
                if row_chain_id is None:
                    continue
                if current_chain_id is not None and int(row_chain_id) == int(current_chain_id):
                    continue
                conflicts.append(f"{row.get('name') or row['id']}(链{int(row_chain_id)})")
            if conflicts:
                joined = "，".join(conflicts[:8])
                if len(conflicts) > 8:
                    joined += f" 等{len(conflicts)}个"
                raise ValueError(f"存在属于其他进化链的精灵：{joined}。严格模式下不允许跨链混编，请先解除原链关系。")

            chain_id: int
            if current_chain_id is not None:
                chain_id = int(current_chain_id)
            else:
                await cur.execute("SELECT COALESCE(MAX(chain_id), 0) + 1 AS next_id FROM pokemon_evolution_chain")
                chain_id = int((await cur.fetchone() or {}).get("next_id") or 1)

            await cur.execute("DELETE FROM pokemon_evolution_chain WHERE chain_id = %s", (chain_id,))
            await cur.execute("UPDATE pokemon SET chain_id = NULL WHERE chain_id = %s", (chain_id,))

            indegree = {node_id: 0 for node_id in node_ids}
            for edge in edges:
                indegree[edge["pokemon_id"]] = indegree.get(edge["pokemon_id"], 0) + 1
            root_ids = [node_id for node_id in node_ids if indegree.get(node_id, 0) == 0]
            for root_id in root_ids:
                await cur.execute(
                    """
                    INSERT INTO pokemon_evolution_chain (chain_id, pre_pokemon_id, pokemon_id, pre_evolution_condition)
                    VALUES (%s, NULL, %s, '')
                    """,
                    (chain_id, root_id),
                )
            for edge in edges:
                await cur.execute(
                    ...
                    (
                        ...
                        edge["pre_pokemon_id"],
                        edge["pokemon_id"],
                        edge.get("pre_evolution_condition", ""),
                    ),
                )

            attach_ids = sorted(node_set)
            attach_placeholders = ", ".join(["%s"] * len(attach_ids))
            await cur.execute(
                f"UPDATE pokemon SET chain_id = %s WHERE id IN ({attach_placeholders})",
                [chain_id, *attach_ids],
            )
        await conn.commit()
        return await _get_evolution_graph_by_chain_id(conn, chain_id)
...
async def delete_evolution_graph_by_chain_id_for_ops(chain_id: int) -> bool:
    pool = await get_pool()
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("DELETE FROM pokemon_evolution_chain WHERE chain_id = %s", (chain_id,))
...
async def _get_evolution_graph_by_chain_id(conn: AsyncConnection, chain_id: int) -> dict:
    async with conn.cursor() as cur:
        await cur.execute(
            """
            SELECT pre_pokemon_id, pokemon_id, pre_evolution_condition
        ...
        rows = await cur.fetchall()
        if not rows:
            return {"chain_id": chain_id, "nodes": [], "edges": []}

        node_ids: set[int] = set()
        root_ids: set[int] = set()
        edges: list[dict] = []
        for row in rows:
            post_id = int(row["pokemon_id"])
            node_ids.add(post_id)
            pre_id = row.get("pre_pokemon_id")
            if pre_id is None:
            ...
            edges.append(
                {
                    "pre_pokemon_id": src,
                    "pokemon_id": post_id,
                    "pre_evolution_condition": (row.get("pre_evolution_condition") or "").strip(),
                }
            )
        ...
        pokemon_rows = await cur.fetchall()
        meta = {int(row["id"]): row for row in pokemon_rows}
        nodes = []
        for node_id in sorted(node_ids):
            ...
            nodes.append(
                {
                    "pokemon_id": node_id,
                    "pokemon_name": (row or {}).get("name", ""),
                    "image_url": build_friend_image_url((row or {}).get("image_lc", ""), (row or {}).get("image", "")),
                    "is_root": node_id in root_ids,
                }
            )
        return {"chain_id": chain_id, "nodes": nodes, "edges": edges}
